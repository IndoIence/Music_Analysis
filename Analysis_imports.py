...
from utils import CONFIG, MyArtist, sanitize_art_name

config_G = CONFIG["Genius_scraping"]

logging.basicConfig(
    filename=config_G["GENIUS_LOGFILE"], level=logging.INFO, format="%(asctime)s - %(levelname)s - %(message)s"
)
...
def scrape_artists_songs(artist_list: list[str]):
    urls = open(CONFIG["all_artists_urls_path"]).read().strip().split()
    # skip comments
    urls_set = {url.lower() for url in urls if url.lstrip()[0] != "#"}
    for artist_id, artist_name in enumerate(tqdm(artist_list)):
        artist = scrape_artist_songs(artist_name)
        if not artist:
            continue
        # check if already saved
        if artist.url.strip().lower() in urls_set:
            logging.info(f"Artist url {artist.url} already saved")
            continue
        urls_set.add(artist.url.strip().lower())
        ...
        # to_pkl(out_path, artist, artist_name)
        pickle.dump(artist, open(out_path / f"{artist_name}", "wb"))
...
def scrape_artist_songs(artist_name: str):
    genius = lyricsgenius.Genius(
        access_token=config_G["GENIUS_BEARER"],
    ...
    try:
        artist = genius.search_artist(
            artist_name, max_songs=config_G["max_songs"], include_features=config_G["features"]
        )
